...
def calculate_massflow_and_velocity(Q_total, p, D, A):
    """Thermal-hydraulics of the saturated phases: mass flow and velocities."""
...
def calculate_friction_factors(p, v_liq, v_vap, D, rho_l, rho_v, eps):
    """Reynolds numbers and friction factors (smooth or Haaland-rough)."""
...
def calculate_friction_factor_colebrook(Re, eps, D, tolerance=1e-6, max_iterations=100):
    """Solve the Colebrook equation iteratively, starting from the smooth-pipe guess."""
    if Re <= 0:
        return 0
...
def calculate_height(rho_l, rho_v, v_l, v_v, f_l, f_v, k_l, k_v, L_l, L_v, D, g=9.81):
    """Pressure balance for the chimney height h (Point 1)."""
...
def calculate_L_for_h_equal_L(rho_l, rho_v, v_l, v_v, f_l, f_v, k_l, k_v, D, g=9.81):
    """Analytical solution for L imposing h = L (Point 2)."""
...
def load_diameter_data(filepath):
    """Load the pipe-size table from the TXT file."""
    try:
...
def solve_for_specific_diameter(od_inch, th_inch, Q_thermal, p, L_max, k_c, k_h, g, eps, find_L=False):
    """Compute h (or required L) for a single user-specified diameter."""
...
def run_optimization_cycle(data_table, Q_thermal, p, L_max, L_liq, L_vap, k_c, k_h, g, eps, find_L=False):
    """Sweep the pipe-size table (Haaland/smooth friction); return valid (OD, value) pairs."""
    plot_x = []
    plot_y = []
    valid_od = None
    ...
    for row in data_table:
        ...
        if not find_L:
            ...
            if 0 < h_i <= L_max:
                plot_x.append(od_i)
                plot_y.append(h_i)
                if valid_od is None:          # keep the first valid diameter found
                    valid_od = od_i
        else:
            L_req = calculate_L_for_h_equal_L(rl_i, rv_i, vl_i, vv_i, fl_i, fv_i, k_c, k_h, D_i, g)
            if L_req is not None and 0 <= L_req <= 100:   # guard against None
                plot_x.append(od_i)
                plot_y.append(L_req)
                if valid_od is None:          # keep the first valid diameter found
                    valid_od = od_i
...
def run_optimization_cycle_colebrook(data_table, Q_thermal, p, L_max, L_liq, L_vap, k_c, k_h, g, eps, find_L=False):
    """Same sweep as run_optimization_cycle but using the Colebrook friction factor."""
    plot_x = []
    plot_y = []
    valid_od = None
...
def plot_results_comparison(od_smooth, h_smooth, od_list_haaland, h_list_haaland, od_list_colebrook, h_list_colebrook, eps):
    """Haaland vs Colebrook comparison with a zoomed inset plot."""
    ...
    # Inset (zoom): width/height are percentages of the main axes
    ax_ins = inset_axes(ax, width="45%", height="45%", loc='upper right', borderpad=3)

    # Same data inside the inset
    ...
    # Zoom limits (8" - 14"), y range taken from the Colebrook curve as reference
    x1, x2 = 8, 14
    ...
    ax_ins.set_yscale('log')   # keep the logarithmic scale in the inset too
    ax_ins.grid(True, alpha=0.2)
    ax_ins.tick_params(axis='both', which='major', labelsize=8)

    # Connector lines between the main plot and the zoom
...
# ---- POINT 3: rough pipes (given roughness), solve for L = h ----
EPSILON_3 = 5.0e-5  # given roughness
# Haaland
...
# ---- Specific diameters to evaluate: (OD, thickness) ----
target_diameters = [
    (8.625, 0.322),
    (10.750, 0.365),
    (12.750, 0.375),
...
# ---- Summary of the minimum working diameters ----
print("\n" + "="*100)
print(f"{'PROJECT SUMMARY: MINIMUM WORKING DIAMETERS':^100}")
print("="*100)
print(f"{'Point 1: Smooth (L=10m)':<40} | Min OD: {min_od_1 if min_od_1 else 'N/A':>10} inch")
print(f"{'Point 2: Smooth (L=h)':<40} | Min OD: {min_od_2 if min_od_2 else 'N/A':>10} inch")
print(f"{'Point 3: Rough (Haaland)':<40} | Min OD: {min_od_3_h if min_od_3_h else 'N/A':>10} inch")
print(f"{'Point 3: Rough (Colebrook)':<40} | Min OD: {min_od_3_c if min_od_3_c else 'N/A':>10} inch")
print("="*100)

# ---- Comparative table for the selected diameters ----
print("\n" + "="*110)
print(f"{'COMPARATIVE TABLE OF SELECTED DIAMETERS':^110}")
print("="*110)

header = f"{'OD [in]':<10} | {'Th [in]':<10} | {'h (Point 1) [m]':<18} | {'L=h (Point 2) [m]':<18} | {'L=h Rough (H) [m]':<18} | {'L=h Rough (C) [m]':<18}"
print(header)
print("-" * 110)

for od, th in target_diameters:
    # Point 1: smooth, L=10, solve for h
    h1_val = solve_for_specific_diameter(od, th, Q_THERM, P_SYS, L_LIM, K_COLD, K_HOT, G_CONST, EPSILON, find_L=False)

    # Point 2: smooth, solve for L = h
    h2_val = solve_for_specific_diameter(od, th, Q_THERM, P_SYS, L_LIM, K_COLD, K_HOT, G_CONST, EPSILON, find_L=True)

    # Point 3: rough (Haaland), solve for L = h
    h3_h_val = solve_for_specific_diameter(od, th, Q_THERM, P_SYS, L_LIM, K_COLD, K_HOT, G_CONST, EPSILON_3, find_L=True)

    # Point 3: rough (Colebrook), computed directly for the specific diameter
    ...
    # Row formatting (N/A if the diameter is not physically feasible)
