...
# PATHS
# =============================================================================
# Cladding temperature profiles produced by Assignment 2
# (run assignment2.py first to (re)generate this CSV).
A2_DIR = Path(__file__).resolve().parent.parent / "assignment2"
CLADDING_T_CSV = A2_DIR / "output_cladding_T.csv"

# Output folder for the plots (assignment3/plots)
PLOTS_DIR = Path(__file__).resolve().parent / "plots"


# =============================================================================
# DATA LOADING & MATERIAL PROPERTIES
# =============================================================================
def load_cladding_temperatures(csv_path=CLADDING_T_CSV):
    """
    Read the CSV produced by Assignment 2 (columns: z_m, T_ci_C, T_co_C).
    Returns (z, T_ci, T_co): axial position [m] and inner/outer cladding wall
    temperatures [deg C].
    """
...
def properties(T):
    """Zircaloy-4 properties (Kye-Ho Tab.2) as a function of T [K]."""
...
def buckling(T, r_avg, t):
    """
    Critical collapse (buckling) pressure, Eq.(1) of the PDF:
        p_cr = E / (4 (1 - nu^2)) * (t / r_avg)^3
    T: cladding temperature in K (scalar or vector); E and nu (Zircaloy-4,
    Kye-Ho Tab.2) are evaluated at that T. Returns p_cr in Pa.
    """
...
def internal_pressure_analysis(sigma_y, r_avg, t):
    """Maximum admissible internal pressure (Mariotte, sigma_h = sigma_y)."""
...
def stress_analysis(r_in, r_out, p_i, p_o):
    """Mechanical stresses (hoop h, radial r, longitudinal l) at the inner and
    outer wall, split into average (primary) and secondary (bending) parts."""
    # Inner wall
    sigma_h_in = (p_i*(r_in**2 + r_out**2) - 2*p_o*r_out**2)/(r_out**2 - r_in**2)
    sigma_r_in = - p_i
    sigma_l_in = (p_i*r_in**2 - p_o*r_out**2)/(r_out**2 - r_in**2)

    # Outer wall
    sigma_h_out = (2*p_i*r_in**2 - p_o*(r_in**2 + r_out**2))/(r_out**2 - r_in**2)
    sigma_r_out = - p_o
    sigma_l_out = (p_i*r_in**2 - p_o*r_out**2)/(r_out**2 - r_in**2)

    # Average (primary) components
    sigma_avg_h = (sigma_h_in + sigma_h_out) / 2
    sigma_avg_r = (sigma_r_in + sigma_r_out) / 2
    sigma_avg_l = (sigma_l_in + sigma_l_out) / 2

    # Secondary (bending) components = local - average
    sigma_h_in_sec = sigma_h_in - sigma_avg_h
    sigma_r_in_sec = sigma_r_in - sigma_avg_r
    sigma_l_in_sec = sigma_l_in - sigma_avg_l

    sigma_h_out_sec = sigma_h_out - sigma_avg_h
    sigma_r_out_sec = sigma_r_out - sigma_avg_r
    sigma_l_out_sec = sigma_l_out - sigma_avg_l

    # Tresca equivalent stress on the average components
    sigma_max_avg = max(abs(sigma_avg_h-sigma_avg_r),
                        abs(sigma_avg_r-sigma_avg_l),
                        abs(sigma_avg_l-sigma_avg_h))
...
def thermal_stress(T_ci, T_co, r_in, r_out):
    """Thermal stresses at the inner and outer wall (T_ci, T_co in K).
    The radial component is zero; hoop and longitudinal components coincide."""
    E, nu, alpha = properties((T_ci + T_co)/2)

    term1 = (E*alpha*(T_ci - T_co))/(2*(1 - nu))/np.log(r_out/r_in)  # MPa, common term
    sigma_h_in = term1*(1-(2*r_out**2)/(r_out**2-r_in**2) * np.log(r_out/r_in))   # MPa
    sigma_h_out = term1*(1-(2*r_in**2)/(r_out**2-r_in**2) * np.log(r_out/r_in))   # MPa
...
def gas_plenum_analysis(T_gas, d_p, D_in, H, p_max):
    """Minimum plenum volume/height to keep the end-of-life gas pressure below
    p_max, accounting for fission gas release and initial impurities."""
    # Data
...
# =============================================================================
def main():
    # ---- Inputs (read inputs.csv in this folder) ----
    inputs, _ = load_inputs()

    D_out_clad = inputs["D_out_clad"]    # m, cladding outer diameter
    thickness  = inputs["s_clad"]        # m, cladding thickness
    D_in = D_out_clad - 2 * thickness    # m, cladding inner diameter
    r_avg = (D_out_clad + D_in) / 4      # m, mean radius = (r_in + r_out)/2
    p_sys = inputs["p_sys"]              # Pa, system pressure
    D_fuel = inputs["D_pellet"]          # m, fuel pellet diameter
    H_active = inputs["H_active"]        # m, active pin height

    # ---- Mechanical properties ----
    sigma_y = 241  # MPa, yield strength
    sigma_u = 413  # MPa, ultimate strength
    S = min(2/3 * sigma_y, 1/3 * sigma_u)  # MPa, allowable stress (Kye-Ho Tab.2)

    # ---- Cladding temperatures from the Assignment 2 output ----
    z, T_ci, T_co = load_cladding_temperatures()
    T_gas = T_ci
    T_c_avg = (T_co + T_ci) / 2          # deg C, wall-average profile along z

    # ---- 1) BUCKLING ANALYSIS: p_cr(z); worst case is the minimum ----
    p_cr = buckling(T_c_avg + 273.15, r_avg, thickness)  # Pa
    i_min = int(np.argmin(p_cr))
    # Collapse check: critical pressure must exceed the external pressure
    if p_cr[i_min] > p_sys:
    ...
    # ---- 3) STRESS ANALYSIS ----
    sigma_max_avg, sigma_in_diz, sigma_out_diz, sigma_in_sec_diz, sigma_out_sec_diz, sigma_avg_diz = \
        stress_analysis(D_in/2, D_out_clad/2, p_i, p_sys)  # Pa
    P_avg = sigma_max_avg / 1e6     # MPa
    # Convert mechanical components Pa -> MPa
    sigma_in_diz = {k: v / 1e6 for k, v in sigma_in_diz.items()}
    sigma_out_diz = {k: v / 1e6 for k, v in sigma_out_diz.items()}
    sigma_avg_diz = {k: v / 1e6 for k, v in sigma_avg_diz.items()}
    sigma_in_sec_diz = {k: v / 1e6 for k, v in sigma_in_sec_diz.items()}
    sigma_out_sec_diz = {k: v / 1e6 for k, v in sigma_out_sec_diz.items()}

    # ASME verification (primary stresses, design conditions): sigma_max <= Sm
    print()
    print("=" * 52)
    ...
    # Use the average stress value P_avg (single value instead of inner/outer)
    name = "Average"
    smax = P_avg
    ...
    # Total stresses = primary mechanical + secondary mechanical + thermal
    sigma_h_in_tot = sigma_avg_diz["h"] + sigma_in_sec_diz["h"] + sigma_th_in_diz["h"]
    ...
    # Tresca (element-wise) for inner and outer: max of the three differences at each z
    ...
    # Scalar comparison on the maximum along z (worst case)
    ...
    # ---- 5) GAS PLENUM SIZING (p_max = Mariotte admissible pressure) ----
    ...
    # 2) Tresca equivalent stress vs 3*Sm
    plt.figure()
    ...
    # 3) Total stresses (mechanical + thermal) per component (h, r, l), inner/outer walls
    plt.figure()
    ...
    # =========================================================================
    # DIAGNOSTIC AUDIT BLOCK (print only, no calculation changes)
